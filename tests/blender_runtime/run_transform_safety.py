"""Blender runtime regression for rotated/off-axis Family member transforms.

The test verifies that family-axis stretching does not introduce matrix shear,
canonical rotation remains stable, 90-degree axis mapping is sensible, and
re-applying the same Family dimensions is idempotent.
...
def main():
    ...
    try:
        ...
        # Explicitly exercise one family-axis stretch while retaining movement
        # semantics on a different axis. This is the case that previously used
        # family_scale @ rotated_basis and could create shear.
        ...
        # A local axis rotated 90 degrees must map to the family axis it now
        # represents instead of blindly assuming local X == family X.
        ...
        # Returning to base dimensions should restore the exact captured matrix,
        # not merely an approximately decomposed equivalent.
        root["bfc_applying"] = True
        try:
