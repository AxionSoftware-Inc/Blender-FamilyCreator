...
def _normalized_basis_axis(matrix3, local_axis):
    axis = Vector((
        float(matrix3[0][local_axis]),
        float(matrix3[1][local_axis]),
        float(matrix3[2][local_axis]),
    ))
    if axis.length <= 1e-12:
        fallback = Vector((0.0, 0.0, 0.0))
        fallback[local_axis] = 1.0
        return fallback
    axis.normalize()
    return axis


def _best_local_to_family_axis_map(basis_matrix):
    """Return a 1:1 local-axis -> family-axis mapping with maximum alignment.

    Basis columns are normalized before scoring, so unapplied/non-uniform local
    scale does not bias the mapping. The six possible 3D permutations guarantee
    a one-to-one assignment, including 90-degree rotations.
    """
    basis = basis_matrix.to_3x3()
    directions = [_normalized_basis_axis(basis, local_axis) for local_axis in range(3)]
    scores = [
        [abs(float(directions[local_axis][family_axis])) for family_axis in range(3)]
...
def _stretch_basis_without_shear(base_matrix, stretch):
    """Scale canonical local basis columns without changing their directions.

    Left-multiplying an anisotropic family-axis scale into a rotated member can
    introduce new shear. Decompose/recompose avoids that for orthogonal sources
    but silently normalizes a source that already contains shear. Scaling the
    captured basis columns directly is safer: orthogonal bases remain
    orthogonal, while existing canonical shear angles are preserved unchanged.
    """
    ...
    local_to_family = _best_local_to_family_axis_map(basis)
    for local_axis in range(3):
        factor = float(stretch[local_to_family[local_axis]])
        if abs(factor - 1.0) <= 1e-12:
            continue
        for row in range(3):
            basis[row][local_axis] = float(basis[row][local_axis]) * factor
    return basis
...
def export_family(root, directory, export_glb=True):
    """Write the base manifest and optionally export primary GLB geometry.

    Typed/package exports normally call this with `export_glb=False` and own the
    wider transaction in `typed.py`. Direct callers still get the same state-safe
    selected-object exporter used everywhere else.
    """
